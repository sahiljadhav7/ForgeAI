...
- **Clerk production keys.** `.env` holds `pk_live` keys, and Clerk only accepts them on `forgeai.lol`. On localhost the `<SignIn>`/`<SignUp>`/`<PricingTable>` widgets render blank (the console shows "Production Keys are only allowed for domain"), and you can't sign in. Every route's `failed` list includes two `400 https://clerk.forgeai.lol/v1/...` entries, which are expected. Signed-in routes (`/workspace`, `/projects`) therefore always 307 to `/sign-in`. Report them as unverified rather than passed. Only the user can fix this, by adding Clerk dev keys in `.env.local`.
